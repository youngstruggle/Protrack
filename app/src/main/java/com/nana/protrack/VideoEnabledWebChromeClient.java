 ...
 * - The invoking activity must call com.lemurian.indisch.VideoEnabledWebChromeClient's onBackPressed() inside of its own onBackPressed().
...
public class VideoEnabledWebChromeClient extends WebChromeClient implements OnPreparedListener, OnCompletionListener, OnErrorListener {
     ...
     * @param activityVideoView A ViewGroup in the activity's layout that will display the video. Typically you would like this to fill the whole layout.
     * @param loadingView A View to be shown while the video is loading (typically only used in API level <11). Must be already inflated and without a parent view.
     * @param webView The owner com.lemurian.indisch.VideoEnabledWebView. Passing it will enable the com.lemurian.indisch.VideoEnabledWebChromeClient to detect the HTML5 video ended event and exit full-screen.
     ...
     * @param callback A com.lemurian.indisch.VideoEnabledWebChromeClient.ToggledFullscreenCallback callback
     */
